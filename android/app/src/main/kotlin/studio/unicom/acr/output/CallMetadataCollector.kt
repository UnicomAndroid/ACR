        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R
            ...
            try {
                ...
                subscriptionInfo?.let {
                    simSlot = it.simSlotIndex + 1
                    val num = it.number
                    if (!num.isNullOrEmpty()) {
                        simPhoneNumber = num
                        Log.d(TAG, "Got SIM ${it.simSlotIndex + 1} phone number: $num")
                    }
                }
                if (simPhoneNumber == null) {
                    Log.d(TAG, "SIM phone number not available")
                }
            } catch (e: Exception) {
                ...
                Log.w(TAG, "Failed to query SIM slot/phone info", e)
            }
        }
