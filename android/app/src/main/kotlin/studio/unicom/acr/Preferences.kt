...
import studio.unicom.acr.extension.DOCUMENTSUI_AUTHORITY
import studio.unicom.acr.extension.safTreeToDocument
...
import studio.unicom.acr.rule.LegacyRecordRule
import studio.unicom.acr.rule.RecordRule
import studio.unicom.acr.template.Template
...
class Preferences(initialContext: Context) {
    companion object {
        ...
        private const val PREF_RECORDING_STATE = "recording_state"
    ...
    var isCallRecordingEnabled: Boolean
        get() = prefs.getBoolean(PREF_CALL_RECORDING, true)
    ...
    var writeMetadata: Boolean
        get() = prefs.getBoolean(PREF_WRITE_METADATA, true)
    ...
    /**
     * Recording state for IPC between InCallService and MainActivity.
     * Written by RecorderInCallService, read by MainActivity.getRecordingState().
     */
    val recordingState: Map<String, Any?>
        get() = mapOf(
            "isRecording" to prefs.getBoolean("rec_state_active", false),
            "phoneNumber" to prefs.getString("rec_state_phone", null),
        )

    fun setRecordingActive(phoneNumber: String?) {
        prefs.edit {
            putBoolean("rec_state_active", true)
            if (phoneNumber != null) putString("rec_state_phone", phoneNumber)
        }
    }

    fun setRecordingInactive() {
        prefs.edit {
            putBoolean("rec_state_active", false)
            remove("rec_state_phone")
        }
    }
