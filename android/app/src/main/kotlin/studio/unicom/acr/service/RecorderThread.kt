...
import studio.unicom.acr.BuildConfig
import studio.unicom.acr.Preferences
import studio.unicom.acr.service.RecorderThread.Companion.BYTES_PER_SAMPLE
import studio.unicom.acr.writeFully
import studio.unicom.acr.extension.deleteIfEmptyDir
import studio.unicom.acr.extension.listFilesWithPathsRecursively
import studio.unicom.acr.extension.phoneNumber
import studio.unicom.acr.extension.threadIdCompat
import studio.unicom.acr.extension.toDocumentFile
import studio.unicom.acr.format.Encoder
import studio.unicom.acr.format.Format
import studio.unicom.acr.output.CallMetadata
import studio.unicom.acr.output.CallMetadataCollector
import studio.unicom.acr.output.CallMetadataJson
import studio.unicom.acr.output.DaysRetention
import studio.unicom.acr.output.FormatJson
import studio.unicom.acr.output.NoRetention
import studio.unicom.acr.output.OutputDirUtils
import studio.unicom.acr.output.OutputFile
import studio.unicom.acr.output.OutputFilenameGenerator
import studio.unicom.acr.output.OutputJson
import studio.unicom.acr.output.OutputPath
import studio.unicom.acr.output.ParameterType
import studio.unicom.acr.output.PhoneNumber
import studio.unicom.acr.output.RecordingJson
import studio.unicom.acr.output.Retention
import studio.unicom.acr.rule.RecordRule
...
import studio.unicom.acr.service.SilenceDetector
...
class RecorderThread(
    ...
    private val manualAudioSources: List<Int>? = null,
    ...
    // Filename and metadata (null in manual mode)
    private val callMetadataCollector = parentCall?.let { CallMetadataCollector(context, it) }
    ...
    private val manualMetadata: CallMetadata? by lazy {
        if (isManual) CallMetadata(
            timestamp = ZonedDateTime.now(), packageName = "manual",
            direction = null, simCount = null, simSlot = null,
            simPhoneNumber = null, callLogName = null, calls = emptyList(),
        ) else null
    }
    val outputPath: OutputPath
        get() = if (callMetadataCollector != null)
            outputFilenameGenerator.generate(callMetadataCollector.callMetadata)
        else
            outputFilenameGenerator.generate(manualMetadata!!)
    ...
    init {
        ...
        sources = manualAudioSources?.toTypedArray() ?: savedFormat.audioSource.sources
    }
    ...
    private fun evaluateRules() {
        ...
        val pc = parentCall!!
        if (pc.details.hasProperty(Call.Details.PROPERTY_CONFERENCE)) {
            for (childCall in pc.children) {
                childCall.details?.phoneNumber?.let { numbers.add(it) }
            }
        } else {
            pc.details?.phoneNumber?.let { numbers.add(it) }
        }
    ...
    override fun run() {
        ...
        try {
            ...
            if (!isManual) evaluateRules() else keepRecording = KeepState.KEEP
            ...
            if (e is PureSilenceException) {
                ...
                val packageName = parentCall?.details?.accountHandle?.componentName?.packageName ?: "manual"
    ...
    private fun writeMetadataFile(path: List<String>, recordingInfo: RecordingInfo?): OutputFile? {
        ...
        try {
            ...
            val metadataJson = CallMetadataJson(
                context,
                callMetadataCollector?.callMetadata ?: manualMetadata!!,
                outputJson,
            )
    ...
    private fun processRetention() {
        ...
        for ((item, itemPath) in directory.listFilesWithPathsRecursively()) {
            ...
            val currentTimestamp = callMetadataCollector?.callMetadata?.timestamp ?: manualMetadata?.timestamp ?: ZonedDateTime.now()
            val diff = Duration.between(timestamp, currentTimestamp)
    ...
    private fun encodeLoop(
        ...
        while (!isCancelled || !buffersFlushed) {
            ...
            silenceDetector.process(bufferOut, 0, nWritten)
        ...
        } else if (silenceDetector.finish().isPureSilence) {
