...
import studio.unicom.acr.Notifications
import studio.unicom.acr.Permissions
import studio.unicom.acr.Preferences
import studio.unicom.acr.R
...
class RecorderInCallService : InCallService(), RecorderThread.OnRecordingCompletedListener {
    ...
    private fun onRecorderExited(recorder: RecorderThread) {
        ...
        updateForegroundState()
        updateRecordingPrefs()
    }

    private fun updateRecordingPrefs() {
        val active = callsToRecorders.entries.firstOrNull()
        if (active != null) {
            prefs.setRecordingActive(active.key.details?.handle?.schemeSpecificPart)
        } else {
            prefs.setRecordingInactive()
        }
    }
