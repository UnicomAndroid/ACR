...
package studio.unicom.acr.format

import android.media.MediaFormat
import android.util.Log
import studio.unicom.acr.Preferences
import studio.unicom.acr.service.RecorderThread
...
sealed class Format {
    ...
    companion object {
        ...
        /** Fallback chain for encoder selection (best quality first). */
        private val fallbackChain: List<() -> Format> = listOf(
            ::FlacFormat, ::OpusFormat, ::AacFormat,
            ::AmrWbFormat, { WaveFormat }, ::AmrNbFormat,
        )

        /** Get preferred format or fall back through the encoder chain. */
        fun getWithFallback(preferredName: String?): Format {
            if (preferredName != null) getByName(preferredName)?.let { return it }
            for (c in fallbackChain) {
                try { val f = c(); Log.i(TAG, "Fallback: ${f.name}"); return f }
        ...
        fun getByName(name: String): Format? {
            all.find { it.name.equals(name, ignoreCase = true) }?.let { return it }
            // Fallback: match by common aliases used by Flutter UI
            return when (name.lowercase()) {
                "opus", "ogg" -> all.find { it is OpusFormat }
                "aac", "m4a" -> all.find { it is AacFormat }
                "flac" -> all.find { it is FlacFormat }
                "wav", "pcm" -> all.find { it is WaveFormat }
                "amr_wb", "amr-wb", "amrwb" -> all.find { it is AmrWbFormat }
                "amr_nb", "amr-nb", "amrnb" -> all.find { it is AmrNbFormat }
                else -> null
            }
        }
        ...
        fun fromPreferences(prefs: Preferences): SavedFormat {
            // Use the saved format if it is (still) valid, else first available.
